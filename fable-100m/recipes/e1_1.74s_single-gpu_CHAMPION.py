...
class FullStepGraph:
    ...
    def capture_mega(self, step_data, total_steps, decay_steps, res_schedule):
        ...
        with torch.cuda.stream(ws):
            ...
            for res_w, wbg_w in ((24, True), (24, False), (28, False), (32, False)):
    ...
    def capture(self, batch):
        ...
        self.tm.fill_(0.3)
        model.train()
        keys = [(24, True), (24, False), (28, False), (32, False)]
        s = torch.cuda.Stream()
...
def _selftest(model, opt1, opt2, fs, batch, wbts):
    ...
    sched = [(0, 24), (1, 24), (2, 24), (10, 24), (11, 24),
             (100, 28), (101, 28), (200, 32), (201, 32)]
    opt2.step_count = 0
    opt2.last_norm_step = 0
    for s, res in sched:
        fs.step(s, res, x, y)
    ...
    for key in ((24, True, False), (24, False, False), (28, False, False), (32, False, False)):
