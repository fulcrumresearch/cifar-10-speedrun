...
hyp = {
    "train_epochs": 8.25,      # the schedule's END — the run is timed to here, and
    ...
    "batch_size": 1536,
    "lowres_epochs": 5,       # first N epochs train on lowres crops
    "lowres": 28,
    "bias_lr": 0.0573,
    "head_lr": 0.5415,
    "wd_base": 1.0418e-06,
...
class CifarLoader:
    ...
    def __iter__(self):
        ...
        crop = getattr(self, "crop_size", self.images.shape[-2])
        if crop < self.images.shape[-2]:
            # low-res epoch: crop from the UNPADDED image (translation range
            # (32-crop)/2 px, same spirit as the 2px translate at full res)
            base = self.proc_images["flip"] if self.aug.get("flip", False) else self.proc_images["norm"]
            images = batch_crop(base, crop)
...
def run_steps(model, loader, optimizer1, optimizer2, decay_steps,
    ...
    for epoch in range(ceil(max_steps / len(loader))):
        model.train()
        loader.crop_size = hyp["lowres"] if epoch < hyp["lowres_epochs"] else 32
        for inputs, labels in loader:
            ...
            if ctx is not None:
                ...
                if step >= max_steps:
                    ...
                    do_val = step % dense_val_every2 == 0 or step > decay_steps
...
def setup():
    ...
    saved_lowres_epochs = hyp["lowres_epochs"]
    # warm the low-res graphs (incl. whiten-bias variants)
    hyp["lowres_epochs"] = 10 ** 9
    run_steps(model, warmup_loader, optimizer1, optimizer2, decay_steps,
              whiten_bias_train_steps, n_warm)
    # warm the full-res graphs (incl. whiten-bias variants)
    hyp["lowres_epochs"] = 0
    run_steps(model, warmup_loader, optimizer1, optimizer2, decay_steps,
              whiten_bias_train_steps, n_warm)
    hyp["lowres_epochs"] = saved_lowres_epochs
    del warmup_loader
