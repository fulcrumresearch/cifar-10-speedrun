# runners/recipes/e5.py — VENDORED from fable-100m/recipes/e5_1.80s_single-gpu.py
# (fable sample e5: a 28->32 resolution curriculum compiled `max-autotune`, so
# inductor auto-cudagraphs the step, with ZERO hand-rolled CUDA graphs — the
# cleanest "curriculum on plain torch.compile" recipe). Driven by
# runners/targets.py (_E5Base); NOT edited — the adapter only orchestrates its
# setup()/reset_all()/train() hooks.
...
hyp = {
    "train_epochs": 8.125,      # the schedule's END — the run is timed to here, and
    ...
    "decay_epochs": 8.0,      # linear LR decay length; tail after this holds lr at floor
    ...
    "batch_size": 1536,
    # per-epoch training resolution; entry i = crop size for epoch i (last
    # entry repeats for any remaining epochs). FLOPs scale with H*W, so early
    # low-res epochs are cheaper; final epochs at 32 match the eval domain.
    "res_schedule": [28, 28, 28, 28, 28, 32, 32, 32, 32],
    "bias_lr": 0.0573,
    "head_lr": 0.5415,
    "wd_base": 1.0418e-06,
    "sgd_momentum": 0.825,
...
class CifarLoader:
    def __init__(self, path, train=True, batch_size=500, aug=None):
        ...
        self.crop_size = self.images.shape[-2]
    ...
    def __iter__(self):
        ...
        if self.aug.get("translate", 0) > 0:
            pad_img = self.proc_images["pad"]
            src_size = self.crop_size + 2 * self.aug["translate"]
            off = (pad_img.shape[-1] - src_size) // 2
            if off > 0:
                pad_img = pad_img[:, :, off:off + src_size, off:off + src_size]
            images = batch_crop(pad_img, self.crop_size)
...
class GlobalMaxPool(nn.Module):
    """Size-agnostic global max-pool (== AdaptiveMaxPool2d(1)). Uses .max(2).values,
    not the amax form that NaN-diverges under inductor. At 28/32px the feature map
    is already 3x3 so this is identical to the original MaxPool2d(3); it additionally
    lets the net run at 24px (where MaxPool2d(3) collapses a 2x2 map to 0x0)."""
    def forward(self, x):
...
class CifarNet(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            ...
            GlobalMaxPool(),  # was nn.MaxPool2d(3); identical at 28/32px, enables 24px
        )
    ...
    def forward(self, x, whiten_bias_grad=True):
        ...
        x = self.layers(x)  # GlobalMaxPool already returns [B, C]
...
def _forward_step_raw(model, inputs, labels, whiten_bias_grad: bool, label_smoothing: float):
...
# Separate compiled instance per training resolution: each sees exactly one
# input shape, so all stay fully static compiled graphs (no dynamic-shape
# recompile in the timed region).
_FWD_CACHE = {}

def get_forward_step(size):
    if size not in _FWD_CACHE:
        _FWD_CACHE[size] = torch.compile(_forward_step_raw, mode="max-autotune", fullgraph=True)
    return _FWD_CACHE[size]

def res_for_epoch(epoch):
    sched = hyp["res_schedule"]
    return sched[epoch] if epoch < len(sched) else sched[-1]
...
class Model(CifarNet):
    ...
    def eval_forward(self, x):
        # Internal test-time ensembling (untimed): add the diagonal 1px
        # translations that the runner's own TTA (mirror + UL/DR translate)
        # does not cover. Pure inference on the given x; logits are genuine.
        logits = self.forward(x)
        px = F.pad(x, (1, 1, 1, 1), "reflect")
        t1 = self.forward(px[:, :, 0:32, 2:34])  # shift up-right
        t2 = self.forward(px[:, :, 2:34, 0:32])  # shift down-left
        return logits + 0.5 * (t1 + t2)
...
def run_steps(model, loader, optimizer1, optimizer2, decay_steps,
              ...
              lr_floor=0.0, use_res_schedule=False):
    ...
    for epoch in range(ceil(max_steps / len(loader))):
        if use_res_schedule:
            loader.crop_size = res_for_epoch(epoch)
        fwd = get_forward_step(loader.crop_size)
        model.train()
        for inputs, labels in loader:
            loss = fwd(model, inputs, labels, step < whiten_bias_train_steps, ls)
            loss.backward()
...
def setup():
    ...
    distinct_sizes = list(dict.fromkeys(hyp["res_schedule"]))
    first_size = hyp["res_schedule"][0]
    warmup_loader.crop_size = first_size
    run_steps(model, warmup_loader, optimizer1, optimizer2, decay_steps,
              whiten_bias_train_steps, n_warm)
    for size in distinct_sizes:
        if size == first_size:
            continue
        warmup_loader.crop_size = size
        run_steps(model, warmup_loader, optimizer1, optimizer2, decay_steps,
              0, 8)
    del warmup_loader
    ...
    train_loader.crop_size = hyp["res_schedule"][0]
