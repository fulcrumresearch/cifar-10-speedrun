...
import os
import sys  # noqa: F401  (kept for byte-parity with the original)
# --- Vendored from base/cifar10_speedrun.py (the genuine 7.65-ep hiverge
#     baseline). SOLE change vs the original: the import-time `open(sys.argv[0])`
#     self-read (an airbench logging leftover; `code` is otherwise unused here)
#     is neutralised so this file is safe to IMPORT as a module under runners/.
#     The runner only ever IMPORTS this and drives its classes via the adapter;
#     the `__main__` block (incl. its torch.cuda._sleep) is never executed in an
#     official run. Keep in sync with base/; see runners/README.md. ---
code = ""
import uuid
